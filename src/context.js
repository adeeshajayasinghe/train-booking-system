...
const AppProvider = ({children}) => {
    ...
    function handleTrainDetails(details){
        console.log(details)    
        setDetails(details);
    }
    function handleOriginalData(original){
        setorginalData(original);
    }
   

    return(
        <AppContext.Provider value={{
            isSidebarOpen, isSubMenuOpen, openSidebar, openSubMenu, closeSidebar, closeSubMenu,
             location, page, firstName, lastName, mobile, NIC, email, password, trainList, priceList, trainID, classIndex,
              trainName, trainNo, passengerCount, OTP, adminEnable, openPopup, date, from, to, classPrice, fullArray, clickedSeats, 
              seatingData, seatNumbers, timeFrom, timeTo, refNumber, className,orginalData,tarinDetails, handleFirstName, handleLastName, handleMobile, handleNIC,
               handleEmail, handlePassword, getTrainList, handleTrainID, handleClassIndex, handleTrainName, handleTrainNo, handlePassengerCount,
                handleOTP, handleAdmin, handlePopup, handleDate, handleFrom, handleTo, handleClassPrice, handleFullArray, handleClickedSeats,
                 handleSeatingData, handleSeatNumbers, handleDepartTime, handleArrivalTime, handleRefNumber, handleClassName,handleOriginalData,handleTrainDetails
