...
export default function PaymentAnaliticsPerRoute() {
  ...
  const Currentmonth = Months[now.getMonth()];
    ...
    const fetchData=async()=>{
      ...
      setlabelSet([]);
      if(date){
       await axios.get(`http://localhost:4000/booking/revenue/pertrain/date/${date}`)
       .then(response=>{
     ...
     const fetchMonthlyData = () => {
      ...
      if (month) {
        ...
        axios.get(`http://localhost:4000/booking/revenue/pertrain/month/${monthNo}`)
          .then(response => {
            const data = response.data;
            console.log(data);
            if (data) {
    ...
    const fetchRangeData=async()=>{
      ...
      setlabelSet([]);
      if(todate && fromdate){
       await axios.get(`http://localhost:4000/booking/revenue/pertrain/date/${fromdate}/${todate}`)
       .then(response=>{
        const data = response.data;
            console.log(data);
            if (data) {
