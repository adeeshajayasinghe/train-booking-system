...
const AdminTrains = () => {
  ...
  const updatehandler= async(e,index)=>{
    ...
    if(tarinDetails.length>orginalData.length){
      try{
        await axios.post('https://stage-pilot-train-booking-system.onrender.com/trains/addtrain',
      {
    ...
    try{
      await axios.put(`https://stage-pilot-train-booking-system.onrender.com/trains/update/${id}`,tarinDetails[index])
    }
    catch(err){
      console.log(err)
    }
 ...
 useEffect(()=>{
  const fetchName=async()=>{
    try{
      const {data}=await axios.get('https://stage-pilot-train-booking-system.onrender.com/trains/getTrainName');
      setTrainName(data)
    }
    catch(err){
      console.log(err)
...
const fetchData=async() => {
  ...
  setIsDelete(false);
        try {
            const res = await axios.get(`https://stage-pilot-train-booking-system.onrender.com/trains/trainDetails/${category}`);
            console.log(res)
              setDetails(res.data);
